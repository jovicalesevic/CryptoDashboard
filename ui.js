...
export function renderRows(tableBody, coins, currencyFormatter, compactFormatter, messages) {
    ...
    coins.forEach((coin) => {
      ...
      <td class="px-4 py-4 font-semibold text-slate-100">${currencyFormatter.format(coin.current_price)}</td>
      <td class="px-4 py-4 font-semibold ${changeTone}">${change.toFixed(2)}%</td>
      <td class="px-4 py-4 text-right text-slate-300">${compactFormatter.format(coin.market_cap)}</td>
      <td class="px-4 py-4">
        ${hasTrend
